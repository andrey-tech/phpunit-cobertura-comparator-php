...
__HALT_COMPILER();
            ...
            WITH RegressedClasses AS (
                -- Шаг 1: Классы, где упало покрытие ЛИБО по строкам, ЛИБО по ветвям
                SELECT
                    m1.class_name,
                    m0.class_line_rate AS old_class_line_rate,
                    m1.class_line_rate AS new_class_line_rate,
                    ...
                    m1.class_branch_rate AS new_class_branch_rate
                FROM metrics m1
                JOIN metrics m0 ON m1.class_name = m0.class_name
                WHERE m1.version = 1
                  AND m0.version = 0
                  AND (m1.class_line_rate < m0.class_line_rate OR m1.class_branch_rate < m0.class_branch_rate)
                GROUP BY m1.class_name
            )
            -- Шаг 2: Выводим ТЕКУЩИЕ методы
            SELECT
                m.file,
                m.class_name,
                m.method_name,
                -- Покрытие по строкам
                (SELECT old.method_line_rate FROM metrics old
                 WHERE old.class_name = m.class_name AND old.method_name = m.method_name AND old.version = 0) AS old_method_line_rate,
                m.method_line_rate AS new_method_line_rate,
                -- Покрытие по ветвям
                (SELECT old.method_branch_rate FROM metrics old
                 WHERE old.class_name = m.class_name AND old.method_name = m.method_name AND old.version = 0) AS old_method_branch_rate,
                m.method_branch_rate AS new_method_branch_rate,
                -- Данные класса
                rc.old_class_line_rate,
                rc.new_class_line_rate,
                ...
                rc.new_class_branch_rate,
                CASE
                    WHEN NOT EXISTS (SELECT 1 FROM metrics old WHERE old.method_name = m.method_name AND old.class_name = m.class_name AND old.version = 0)
                        THEN 'НОВЫЙ МЕТОД'
                    ELSE 'Старый метод'
                END AS method_status
            FROM metrics m
            JOIN RegressedClasses rc ON m.class_name = rc.class_name
            WHERE m.version = 1

            UNION ALL

            -- Шаг 3: Добавляем УДАЛЕННЫЕ методы
            SELECT
                old.file,
                old.class_name,
                old.method_name,
                ...
                rc.old_class_branch_rate,
                rc.new_class_branch_rate,
                'УДАЛЕННЫЙ МЕТОД' AS method_status
            FROM metrics old
            JOIN RegressedClasses rc ON old.class_name = rc.class_name
            WHERE old.version = 0
              AND NOT EXISTS (
                  SELECT 1 FROM metrics current
                  WHERE current.class_name = old.class_name
            ...
            WITH RegressedClasses AS (
                -- Шаг 1: Находим только те классы, где общее покрытие снизилось
                SELECT
                    m1.class_name,
                    m0.class_line_rate AS old_class_line_rate,
                    m1.class_line_rate AS new_class_line_rate
            ...
            -- Шаг 2: Выводим ТЕКУЩИЕ методы (старые и новые)
            SELECT
                m.file,
                m.class_name,
                m.method_name,
                (SELECT old.method_line_rate FROM metrics old
                 WHERE old.class_name = m.class_name
                   AND old.method_name = m.method_name
                   AND old.version = 0) AS old_method_line_rate,
                m.method_line_rate AS new_method_line_rate,
                rc.old_class_line_rate,
                rc.new_class_line_rate,
                CASE
